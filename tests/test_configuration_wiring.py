...
def test_core_url_wiring():
    ...
    assert reverse("device-identify") == "/api/v1/devices/identify/"
    ...
    assert resolve("/api/v1/devices/identify/").view_name == "device-identify"
...
def test_rest_framework_and_schema_wiring():
    ...
    # Env-configured per environment (SSM in prod = foxugly.com); just assert it's wired.
    assert settings.INBOUND_EMAIL_DOMAIN
