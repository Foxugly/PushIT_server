...
- [x] **Accusés de réception — Phase 2 « livré OS » best-effort → ABANDONNÉE (won't-do, 2026-06-15)**.
  Décision Renaud : on en reste à la Phase 1 « ouvert/vu ». La Phase 2 aurait imposé de passer FCM en
  **data-only priorité haute** (`notifications/push.py`) → changement du format des pushes de **toute la
  flotte** + **casse de l'affichage iOS** du stub, pour un accusé « livré » de toute façon **approximatif**
  (force-stop/Doze/iOS). Rapport risque/valeur jugé défavorable. Ne pas reproposer. Endpoint
  `/{id}/delivered/` non construit (et ne le sera pas).
